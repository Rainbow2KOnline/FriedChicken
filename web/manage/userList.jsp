<%@ page language="java" contentType="text/html; charset=UTF-8"
         pageEncoding="UTF-8"%>
<%@ page import="java.util.List,com.javabean.User" %>
<html>
<head>
...
<div class="rightinfo">
  ...
  //设定当前页数（pageNo）和每页数据条数（pageSize）
  此处需要自己填写class和scope 截图
  <jsp:useBean id="pagination"
               class="com.javabean.UserPageBean" scope="session">
  ...
  <table class="imgtable">
    ...
    <tr height="50px">
      ...
      此处需要自己填写内容 截图
      <td><%=user.getUserLogname()%>(用户登录名称)</td>
      <td><%=user.getUserRealname()%>(用户真实名称)</td>
      <td><%=user.getUserEmail()%>(用户邮件)</td>
